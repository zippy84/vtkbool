...
pat = re.compile(r'([a-z]?)\s*([\d\-.,]+)', flags=re.I)
...
print('M%sZ' % ' L'.join(map(lambda pt: ','.join(map(str, pt)), pts)))
...
for a, b in zip(pts, _pts):
    a[0] -= b[0]
    a[1] -= b[1]

pts.reverse()

print('m%sz' % ' '.join(map(lambda pt: ','.join(map(str, pt)), pts)))
